function [SEIQRDPm,InitialStates] = fitSEIRDQP_Mobility(fitBegin,iter,mod,method);
...
alpha    = 1.0;
beta     = 1.0;        % [Days] 
sigma    = 4;      % [Days] Latent period
delta    = 3;       % [Days] Quaratine period
gamma0   = 1.5;
gamma1   = 0.05; % was 
gammaTau = 90;
mu       = 1E-3;
...
FileName             = 'SEIQRDP_ODE_Mobility';              % File describing the SIR model structure
...
SEIQRDPinit.Parameters(1).Maximum = 2;   
...
SEIQRDPinit.Parameters(7).Minimum = 5;   % [1/days] 
...
SEIQRDPinit.Parameters(8).Maximum = 0.1;    % [1/days] 
...
if strcmp(method,'lsqnonlin')
 ...
 optEst.SearchOption.FunctionTolerance = 1E-5;
 optEst.SearchOption.StepTolerance     = 1E-5;   
 optEst.SearchOption.MaxIter = iter;   
end
optEst.OutputWeight=diag([50,5,10]);
%optEst.Regularization.Lambda = 1.00001;
optEst.Regularization.Nominal = 'model';
%optEst.Regularization.Nominal = 'zero';

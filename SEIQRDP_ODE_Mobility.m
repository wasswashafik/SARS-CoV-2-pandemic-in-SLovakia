...
function [dx y] = SEIQRDP_ODE_Mobility(t, x, u, alpha0,beta,sigma,delta,gamma0,gamma1,gammaTau, mu, varargin)

persistent firstFlag Mobility
...
if t>=length(Mobility)
    alpha=alpha0*(0.005);
else
    alpha=alpha0*(interp1(1:length(Mobility),Mobility,t,'spline'));
    if alpha<0
        alpha=0;
    end
end
